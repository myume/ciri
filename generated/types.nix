...
in
  with lib.types; rec {
    ...
    ClickMethod = mkOption {
      type = enum ["clickfinger buttonareas"];
    };
    AccelProfile = mkOption {
      type = enum ["adaptive flat"];
    };
    ScrollMethod = mkOption {
      type = enum ["noscroll twofinger edge onbuttondown"];
    };
    TapButtonMap = mkOption {
      type = enum ["leftrightmiddle leftmiddleright"];
    };
    ...
    WarpMouseToFocusMode = mkOption {
      type = enum ["centerxy centerxyalways"];
    };
    ...
    ModKey = mkOption {
      type = enum ["ctrl shift alt super isolevel3shift isolevel5shift"];
    };
    Input = submodule {
      options = {
        keyboard = mkOption {
    ...
    Transform = mkOption {
      type = enum ["normal _90 _180 _270 flipped flipped90 flipped180 flipped270"];
    };
    Position = submodule {
      options = {
        x = mkOption {
    ...
    GradientRelativeTo = mkOption {
      type = enum ["window workspaceview"];
    };
    GradientColorSpace = mkOption {
      type = enum ["srgb srgblinear oklab oklch"];
    };
    HueInterpolation = mkOption {
      type = enum ["shorter longer increasing decreasing"];
    };
    ...
    PresetSize = mkOption {
      type = enum ["proportion fixed"];
    };
    DefaultPresetSize = submodule {
      options = {
    ...
    CenterFocusedColumn = mkOption {
      type = enum ["never always onoverflow"];
    };
    ColumnDisplay = mkOption {
      type = enum ["normal tabbed"];
    };
    Struts = submodule {
      options = {
        left = mkOption {
    ...
    BlockOutFrom = mkOption {
      type = enum ["screencast screencapture"];
    };
    RelativeTo = mkOption {
      type = enum ["topleft topright bottomleft bottomright top bottom left right"];
    };
    ...
    Trigger = mkOption {
      type = enum ["keysym mouseleft mouseright mousemiddle mouseback mouseforward wheelscrolldown wheelscrollup wheelscrollleft wheelscrollright touchpadscrolldown touchpadscrollup touchpadscrollleft touchpadscrollright"];
    };
    Key = submodule {
      options = {
        trigger = mkOption {
